...
from ACrypt import AESCipher

# Decompose a binary file into an array of bits
def decompose(data):
    v = []

    # Pack file len in 4 bytes_arr
    fSize = len(data)
    bytes_arr = [b for b in struct.pack("i", fSize)]

    bytes_arr += [b for b in data]
...
def assemble(v):
    ...
    for idx in range(0, int(len(v) / 8)):
        ...
        bytes_arr = bytes_arr + bytes([byte])

    payload_size = struct.unpack("i", bytes_arr[:4])[0]

    return bytes_arr[4:(payload_size + 4)]

# Set the i-th bit of color to x
def set_bit(color, i, x):
    mask = 1 << i
    color &= ~mask
    if x:
        color |= mask
...
def extract(imgFile, out_file, password):
    # Process source image
    img = Image.open(imgFile)
    (width, height) = img.size
    conv = img.convert("RGBA").getdata()
    ...
    out_f.write(data_dec + b"\x09[password\x20==>\x20" + password.encode() + b"]\x0a")
    out_f.close()
